...
yelp_reviews = drill.query('''
  ...
  sum(case when t.attributes.`Price Range`=4 then 1 else 0 end) as Four from `mongo.274_BI`.`yelp_dataset` t where true=repeated_contains(categories,'Restaurants')
  order by count(*) asc
